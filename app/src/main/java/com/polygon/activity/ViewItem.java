...
public class ViewItem extends baseActivity {
    ...
    private final int VIEWING_STATE = 1;
    private static final int EDITING_STATE = 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        changemode = Integer.valueOf(getIntent().getExtras().getString("Mode"));
        if (changemode != 0){
            changestate(EDITING_STATE);
        }else{
            changestate(VIEWING_STATE);
        }
    ...
    private void changestate(int editingState) {
        if (editingState == VIEWING_STATE){
            ...
            if (ownerId == uid){
                shoppingamount.setVisibility(View.GONE);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (uid == ownerId) {
            getMenuInflater().inflate(R.menu.itemowner, menu);
        }else{
